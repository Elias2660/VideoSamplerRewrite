...
def sample_video(
    ...
    try:
        ...
        video_frame_array = np.empty((num_frames, height, width, 3), dtype=np.uint8)
        for i in range(num_frames):
            ret, frame = cap.read()
            if not ret:
                # fewer frames than advertised; adjust if necessary
                video_frame_array = video_frame_array[:i]
                break
            video_frame_array[i] = frame
